    ...
    while(worker->active){
        ...
        if(node == NULL){
            ...
            if(b == -1){
                ...
                fprintf(stderr, "[scheduler %ld]: no best fit found for request: %d. putting job in rescheduling queue.\n", pthread_self(), node->value->request);
            }else{
                // best-fit found: time to schedule the task
                printf("best fit index: %d\n", b);
                ...
                printf("[scheduler %ld]: worker %d current resources: %d\n", pthread_self(), b, best_fit->current_resources);
